...
FRAME_DB_ROOT = r"/home/sk/deepfake_project/data/dataset/frame_db"
...
class FrameDataset(Dataset):
    def __init__(self, frame_root, split_json, k=16):
    ...
    def __getitem__(self, idx):
        ...
        frame_dir = os.path.join(self.frame_root, rel_stem)

        frame_files = [os.path.join(frame_dir, f"frame_{i:02d}.jpg") for i in range(self.k)]
...
model = DeepfakeDetector3D(pretrained=True).to(device)
...
for epoch in range(num_epochs):
  ...
  val_loss, val_acc, val_f1, val_auc = eval_model(model, val_loader, device, criterion)
  ...
  if val_auc > best_auc:
    best_auc = val_auc
    counter = 0
    torch.save(model.state_dict(), "frames/best_model.pth")
  ...
  torch.save(model.state_dict(), f"frames/checkpoint_epoch_{epoch+1}.pth")



model_best = DeepfakeDetector3D(pretrained=False)

checkpoint = torch.load('frames/best_model.pth', map_location=device)
